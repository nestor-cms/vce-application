				...
				if ($this_type->file_upload()) {
					...
					foreach ($media_type as $mime_type=>$class_name) {
						...
						if (preg_match('/([a-z]*\/{1})\.\*$/',$mime_type, $match)) {
							$accept .= ',' . str_replace(',',',' . $match[1], $file_extensions['extensions']);
						} else {
							$accept .= ',' . $mime_type;
						}
					}
...
EOF;
	...
	public static function add_file_uploader($recipe_component, $page) {
		...
		$page->site->add_script(dirname(__FILE__) . '/js/jquery.fileupload.js', 'jquery jquery-ui');
	
		// add javascript to page
		$page->site->add_script(dirname(__FILE__) . '/js/script.js');
		
		// add style
		$page->site->add_style(dirname(__FILE__) . '/css/style.css', 'media-style');
		...
		if (defined('MAX_FILE_LIMIT')) {
			...
			$file_size_limit = $page->site->convert_to_bytes('4G');
		}
...
<div id="progressbar-title">Upload In Progress</div>
<div id="progressbar-block">
<div id="progressbar-block-left"><div id="progressbar"><div id="progress-chunks" style="position:absolute;padding-left:5px;"></div></div></div>
...
<div id="verifybar-container">
<div id="verifybar-title">Upload Completed</div>
<div id="verifybar"><div id="verify-chunks" style="position:absolute;padding-left:5px;"></div></div>
<div id="verifybar-label">Verifying File</div>
</div>
...
<label for="fileupload" class="ignore">
<div class="link-button">Browse</div> Choose File To Upload
</label>

<input id="fileupload" type="file" name="file" path="$page->media_upload_path" accept="" file_size_limit="$file_size_limit" chunk_size="$chunk_size" style="display:none;">
...
<div id="start-upload" class="link-button" href="javascript:;">Upload</div> <div id="cancel-upload" class="link-button cancel-button" href="">Cancel</div>
</div>
