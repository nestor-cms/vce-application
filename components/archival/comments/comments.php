...
class Comments extends Component {
	...
	public function as_content($each_component, $page) {
		...
		//add stylesheet
		$page->site->add_style(dirname(__FILE__) . '/css/style.css', 'comments-style');
		
		$created_at = date("F j, Y, g:i a", $each_component->created_at);
		
		$site_url = defined('ASSETS_URL') ? ASSETS_URL : $page->site->site_url;
...
EOF;
		...
		if ($each_component->created_by == $page->user->user_id || $page->user->role_id == 1) {
...
<form id="add_comments" class="asynchronous-form" method="post" action="$page->input_path" autocomplete="off">
<input type="hidden" name="dossier" value="$dossier_for_update">
<label>
<textarea name="text" class="textarea-input" tag="required" placeholder="Enter Your Comment">$comment_edit_text</textarea>
...
EOF;
			...
			// add dossier, which is an encrypted json object of details uses in the form
			$dossier_for_create = $page->generate_dossier($dossier);
...
<label>
<textarea name="text" class="textarea-input" tag="required" placeholder="Enter Your Comment"></textarea>
<div class="label-text">
<div class="label-message">Comment</div>
<div class="label-error">You cannot post an empty comment</div>
</div>
...
EOF;
			...
			if ((!empty($each_component->created_by) && $each_component->created_by == $page->user->user_id) || $page->user->role_id == 1) {
...
<div class="pipe"></div>
<div class="link-inline delete-comment" comment="$each_component->component_id" action="$page->input_path" dossier="$dossier_for_delete">Delete</div>
EOF;
...
EOF;
	...
	public function add_component_finish($each_component, $page) {
		...
		if (!empty($page->content->$add_component)) {
  			$page->content->add('main',$page->content->$add_component);
  		}
	...
	public function add_component($recipe_component, $page) {
		...
		// user image
		$user_image = $site_url . '/vce-application/images/user_' . ($page->user->user_id % 5) . '.png';
...
<form id="add_comments" class="asynchronous-comment-form" method="post" action="$page->input_path" combar="combar-$recipe_component->parent_id" autocomplete="off">
<input type="hidden" name="dossier" value="$dossier_for_create">
...
<div class="comment-remote-container remote-container">
	...
	<form id="add_comments_remote" class="asynchronous-comment-form" method="post" action="$page->input_path" combar="combar-$recipe_component->parent_id" autocomplete="off">
	<input type="hidden" name="dossier" value="$dossier_for_create">
	<input type="hidden" name="title" value="comment">
	<label>
...
EOF;
		...
		$page->content->add($location, $content);
