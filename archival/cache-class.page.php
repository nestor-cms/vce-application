...
class Page {
	...
	function __construct() {
		...
		// passing site object into page object
		global $site;
		$this->site = $site;
		
		// passing user object into page object
		global $user;
		$this->user = $user;
		
		// passing content object into page object
		global $content;
		$this->content = $content;

		// check that http_host and PHP_URL_HOST match
		if ($_SERVER['HTTP_HOST'] != parse_url($site->site_url, PHP_URL_HOST) && !parse_url($site->site_url, PHP_URL_PORT)) {
		 	header('location: ' . parse_url($site->site_url, PHP_URL_SCHEME) . '://' . parse_url($site->site_url, PHP_URL_HOST) . $_SERVER['REQUEST_URI']);
		}
		
		// remove extra / at end of url
		if (preg_match('/\/{2,}$/',$_SERVER['REQUEST_URI'])) {
		 	header('location: ' . parse_url($site->site_url, PHP_URL_SCHEME) . '://' . $_SERVER['HTTP_HOST'] . rtrim($_SERVER['REQUEST_URI'],'/') . '/');
		}
		...
		if (defined('QUERY_STRING_INPUT')) {
			...
			if (isset($full_requested_url[1])) {
				$query_string = array();
				$full_query_string = $full_requested_url[1];
				$query_string_parts = explode('&', stripslashes(filter_var($full_query_string)));
				foreach($query_string_parts as $each_part) {
					$each_part_exploded = explode('=', $each_part);
					if (count($each_part_exploded) == 2) {
						$query_string[$each_part_exploded[0]] = $each_part_exploded[1];
					}
				}
				$this->query_string = json_encode($query_string);
			}
		}
		
		// get the trimmed site url path
		$site_url = trim(parse_url($site->site_url, PHP_URL_PATH ), '/');
		...
		if (isset($site->hooks['page_requested_url'])) {
			foreach($site->hooks['page_requested_url'] as $hook) {
				call_user_func($hook, $requested_url, $this);
			}
		}
		...
		if (!defined('QUERY_STRING_INPUT')) {
			if (isset($full_requested_url[1])) {
				header('location: ' . $site->site_url . '/' . $requested_url);
			}
		}
		...
		if (isset($_SESSION['add_attributes'])) {
			foreach (json_decode($_SESSION['add_attributes'],true) as $key=>$value) {
				// if there is a persistant value set
				if ($key == 'persistant') {
					$persistant = $value;
					foreach ($persistant as $persistant_key=>$persistant_value) {
						$this->$persistant_key = $db->clean($persistant_value);
					}
				} else {
					// normal value
			...
			// rewrite if persistant value had been set
			if (isset($persistant)) {
				$_SESSION['add_attributes'] = json_encode(array('persistant' => $persistant));
			}
		}
		...
		if (!empty($requested_url)) {
			...
			// database query that would include cache			
			$query = "SELECT * FROM " . TABLE_PREFIX . "components INNER JOIN " . TABLE_PREFIX . "components_meta ON " . TABLE_PREFIX . "components.component_id = " . TABLE_PREFIX . "components_meta.component_id WHERE " . TABLE_PREFIX . "components.url='" . $requested_url . "' AND (" . TABLE_PREFIX . "components_meta.meta_key='Type' OR " . TABLE_PREFIX . "components_meta.meta_key='Cache')";

			// call to database, grab first array item because there should only be one
			$requested_location = $db->get_data_object($query);
		
		}
		
if (!isset($requested_location[1]->meta_value)) {
		...
		if (isset($site->hooks['page_construct_object'])) {
			foreach($site->hooks['page_construct_object'] as $hook) {
				call_user_func($hook, $requested_component, $this);
			}
		}
		
		// add basics to object
		$this->requested_id = $requested_component->component_id;
		$this->requested_url = $requested_component->url;
		...
		// and unset to clean things up
		unset($this->recipe);
		
		// for debugging, can print recipe to screen
		// $site->dump($recipe);
		
		// $site->dump($this->requested_id);
...
// 		
// 		$records[0] = array(
// 		'component_id' => $this->requested_id,
// 		'meta_key' => 'cache', 
// 		'meta_value' => serialize($something),
// 		'minutia' => null
// 		);
// 
// 		$db->insert('components_meta', $records);
// 		
// 		// $after_test = json_decode($records);
// 		
// 		// get homepage
// 		$query = "SELECT meta_value FROM " . TABLE_PREFIX . "components_meta WHERE component_id='" . $this->requested_id . "' AND meta_key='cache'";
// 		$requested = $db->get_data_object($query);
// 		
// 		$site->dump($requested[0]->meta_value);
// 		
// 		$cache = unserialize($requested[0]->meta_value);
// 		
// 		$site->dump($cache,'9c3');
...
$cache = unserialize($requested_location[1]->meta_value);
...
$recipe = $cache['recipe'];
		...
		self::build_content($this->components, $recipe, $this->requested_id);
	 ...
	 * this is done from the component which is beeing accessed by url backwards to the start of the recipe
	...
	private function get_components($page_id, $requested_id, $components) {
		...
		if (!isset($components['requested_location'])) {
			...
			$requested_component = $db->get_data_object($query)[0];
		...
		foreach ($components_meta as $each_meta) {
			...
			if (!isset($this->template) && $each_meta->meta_key == "template") {
				...
				if (is_file(BASEPATH .'vce-content/themes/' . $this->site->site_theme . '/' . $each_meta->meta_value)) {
			...
			if ($each_meta->meta_key == "recipe") {
				...
				// load hooks
				if (isset($this->site->hooks['page_add_recipe'])) {
					foreach($this->site->hooks['page_add_recipe'] as $hook) {
			...
			$requested_component->$key = $db->clean($each_meta->meta_value);
		...
		// load hooks
		if (isset($this->site->hooks['page_get_components'])) {
			foreach($this->site->hooks['page_get_components'] as $hook) {
				call_user_func($hook,$requested_component,$components,$this);
			}
		}
		...
		if (isset($activated_components[$requested_component->type])) {
			...
			// load class to fire check_access function for each component
			// this effects children of the component
		...
		$find_sub_componets = $check->find_sub_components($requested_component, $this, $components, $sub_components = array());

		// if component has parent id, recursive call to this function
		if (isset($requested_component->parent_id) && $requested_component->parent_id != 0 && $find_sub_componets) {
		...
		// check that access is allowed for sub components
			...
			if ($find_sub_componets) {
				// get sub-components
				$nested_components = self::get_sub_components($requested_id, $requested_id, $components, $sub_components = array(), false);
	 ...
	 */
	private function get_sub_components($current_id, $parent_id, $components, $sub_components, $sub_url) {
	
		global $db;

		// get children of current_id
		$query = "SELECT * FROM  " . TABLE_PREFIX . "components WHERE parent_id='" . $current_id . "' ORDER BY sequence ASC";
		$requested_components = $db->get_data_object($query);
		...
		if (!empty($requested_components)) {
			...
			foreach ($requested_components as $each_key=>$each_component) {
				...
				$query = "SELECT meta_key, meta_value, minutia FROM  " . TABLE_PREFIX . "components_meta WHERE component_id='" . $each_component->component_id . "' ORDER BY meta_key";
				$components_meta = $db->get_data_object($query);
				...
				foreach ($components_meta as $each_meta) {
					...
					$requested_components[$each_key]->$key = $db->clean($each_meta->meta_value);
				...
				if (empty($each_component->url)) {
					unset($requested_components[$each_key]->url);
				}
				
			}

			// load hooks
			if (isset($this->site->hooks['get_sub_components'])) {
				foreach($this->site->hooks['get_sub_components'] as $hook) {
					$requested_components = call_user_func($hook,$requested_components,$sub_components,$this);
				}
			}

			// anonymous function to place requested components into multidimentional array of sub components
			...
			foreach ($requested_components as $each_key=>$each_component) {
				...
				$recursive = $access->find_sub_components($each_component, $this, $components, $sub_components);
				...
				if ($recursive) {
					...
					if (isset($this->recipe[0]['full_object'])) {
						$recursive = true;
					// check for nested components with the same type
					// the purpose of this is if you have a several branches of differnet depths
				...
				if ($recursive) {
					// our recursive call
					self::get_sub_components($each_component->component_id, $each_component->parent_id, $components, $sub_components, true);
				}	
	...
	private function build_content($components, $recipe, $requested_id, $linked = false, $recipe_tracker = array()) {
		...
		foreach ($components as $each_component_key=>$each_component) {
			...
			// load hooks
			// build_content_loop
			if (isset($this->site->hooks['build_content_loop'])) {
				foreach($this->site->hooks['build_content_loop'] as $hook) {
					call_user_func($hook, $each_component, $linked);
				}
			}

			// anonymous function to find sub componets recipe for a given component
					...
					if ($recipe_item['type'] == $component->type) {
						...
						if ($match) {
							...
							if (isset($recipe_item['components'])) {
								...
								// sort though and add content_create value if it doens't exist on this level
			...
			if (isset($each_component->url)) {
				...
				if ($linked === false) {
					...
					$this_component->as_link($each_component, $this);
					continue;
				}
			}
			...
			if ($this_component->check_access($each_component, $this) || (isset($each_component->recipe['content_edit']) && $each_component->recipe['content_edit'] == 'roles')) {
				
				// normal component layout
				// as_content can be used to stop the build if false is returned by the component method
				$as_content = $this_component->as_content($each_component, $this);
				
				// this currently does not carry forward to sub components
				if ($this_component->allow_sub_components($each_component, $this)) {
					...
					if (!isset($this->prevent_sub_components)) {
						...
						$this_component->recipe_components($each_component, $this);
				...
				if (!isset($each_component->prevent_editing) || $each_component->prevent_editing === false) {
					// call to edit_component function within component
					// at some point this can be updated to call to revise_component
				...
				if (isset($sub_components) && $as_content !== false) {
					...
					if (isset($this->site->hooks['build_content_callback'])) {
						foreach($this->site->hooks['build_content_callback'] as $hook) {
							$sub_components = call_user_func($hook, $sub_components, $each_component, $this);
						}
					}

					// check if build_sub_components is true
					if ($this_component->build_sub_components($each_component, $this)) {
				...
				if (isset($each_component->parent_id)) {
					...
					// as content finish
					$this_component->as_content_finish($each_component, $this);
				...
				for ($key = 0;$key < count($this->components);$key++) {
					...
					if ($this->components[$key]->component_id == $each_component->component_id && isset($this->components[$key]->repudiated_url)) {
						...
						header('location: ' . $this->site->site_url . '/' . ltrim($this->components[$key]->repudiated_url, '/'));
					}
				}
		...
		if (!empty($top_instance)) {
			$top_instance->as_content_finish((object) $top_object, $this);
		}
	 ...
	 */
	public function get_children($current_id, $parent_id = null, $component = array(), $sub_components = array(), $sub_url = false) {
		return self::get_sub_components($current_id, $parent_id, $component, $sub_components, $sub_url);
	}
	...
	public function display_components($components, $recipe = null, $requested_id = null, $linked = false, $recipe_tracker = array()) {
		// $requested_id should be the component where you want things sub out, meaning the sub_recipe add_component you want available
		self::build_content($components, $recipe, $requested_id, $linked, $recipe_tracker);
		return;
	}
	...
	public function find_url($component_id) {
		...
		$requested_component = $db->get_data_object($query);
		
		if (isset($requested_component[0]->url) && strlen($requested_component[0]->url) > 0) {
			return $this->site->site_url . '/' . $requested_component[0]->url;
	...
	public function can_add($each_component) {
		...
		if (isset($each_component->content_create)) {
			if (in_array($this->user->role_id,explode('|',$each_component->content_create))) {
				return true;
			}
		} else {
		...
		// user role_id is contained within content_create, but this time it's contained within recipe
		if (isset($each_component->recipe['content_create'])) {
			if (in_array($this->user->role_id,explode('|',$each_component->recipe['content_create']))) {
				return true;
			}
		}
	...
	public function can_edit($each_component) {
		...
		if (isset($each_component->created_by) && $each_component->created_by == $this->user->user_id) {
			return true;
		}
		
		// content_edit = roles and user->role_id is in content_create
		if (isset($each_component->recipe['content_edit']) && isset($each_component->recipe['content_create'])) {
        	if ($each_component->recipe['content_edit'] == "roles" && in_array($this->user->role_id,explode('|',$each_component->recipe['content_create']))) {
				return true;
			}
		}
	...
	public function can_delete($each_component) {
		...
		if (isset($each_component->prevent_delete)) {
			...
			if (!isset($each_component->prevent_delete_roles)) {
				return false;
			// specific roles
			} else if (in_array($this->user->role_id,explode('|',$each_component->prevent_delete_roles))) {
				return false;
			}
		}
        ...
        if (isset($each_component->recipe['content_delete']) && isset($each_component->recipe['content_create'])) {
        	if ($each_component->recipe['content_delete'] == "roles" && in_array($this->user->role_id,explode('|',$each_component->recipe['content_create']))) {
				return true;
			}
		}
